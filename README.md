...
4. **Import phase** - a second, fresh container runs python3 -c "import <module>" or node -e "require('<name>')" in the same network. Records to import.json. Note: the package is not pre-installed in the import container, so import-only payloads are currently not caught (see Limitations).
...
| pkgids/analyze.py | Stub for future static/dynamic analysis of captured events. |
| pkgids/score.py | Stub for future numeric scoring model. |
| pkgids/report.py | Stub for future structured reporting. |
...
This builds sdists into corpus/dist/ and writes data/corpus_samples.csv. The build script uses a before/after glob diff to find the actual .tar.gz filename regardless of how setuptools normalizes it (hyphens to underscores).

---

## Limitations and Future Work

**Known false negative: import-time callbacks.** canary-import-callback puts its payload in __init__.py. pkgids runs install and import in separate containers. The import container is fresh with the package not installed, so import canary_import_callback fails with ModuleNotFoundError and the callback never fires. Catching import-time payloads requires a combined install-then-import test inside a single container. This is the one missed case in the corpus.
...
**Simple prediction rule.** The current heuristic is: network_activity=True means malicious, install timeout means malicious, otherwise benign. There is no scoring model, no Zeek-based protocol analysis, and no static feature extraction beyond the archive member list. analyze.py and score.py are stubs for future work.
